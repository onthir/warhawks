...
def checkLimit(*args):
    ...
    if len(value) > 2: pin_number_var.set(value[:4])

pin_number_var = tk.StringVar()
pin_number_var.trace('w', checkLimit)
...
def log_in():
    ...
    dire = account_number_var.get() + ".txt"
    ...
    if account_number_var.get() == a_no and pin_number_var.get() == a_pin:
...
def create_login_screen():
    ...
    heading = tk.Label(win, text="FedUni Banking", font=('arial 32'))
    heading.grid(row=0, columnspan=5)

    # ----- Row 1 -----

    # Acount Number / Pin label here
    account_info = tk.Label(win, text='Account Number / Pin')
    account_info.grid(row=1, column=0)
    # Account number entry here
    account_number_entry.grid(row=1, column=1)
    # Account pin entry here
    account_pin_entry.grid(row=1, column=3)
    ...
    btn_clear = tk.Button(win, text="Cancel/Clear", width=20, height=2, bg='red', activebackground='red')
    btn_clear.grid(row=5, sticky='W')
    
    # Button 0 here

    # Login button here. 'bg' and 'activebackground' should be 'green'). Button calls 'log_in' function.
    btn_login = tk.Button(win, text="Login", width=20, height=2, bg='green', command=log_in, activebackground='green')
    btn_login.grid(row=5, column=3, sticky='W')
